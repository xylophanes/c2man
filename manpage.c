/* $Id: manpage.c,v 1.1 2024-06-03 09:56:30 behdad Exp $
...
static void output_identifier_description(const char      *comment,                      /* comment for this identifier */
					  ...
					  const DeclSpec  *decl_spec,                    /* Declarator specification    */
                                          const Declarator *declarator)                  /* Declarator                  */
                                                                                         /*-----------------------------*/
{
	...
	if (entries > 20)
	{
	    ...
	    (void)sprintf(entries_s, "%d", entries);
...
/*-------------------------------------------------------*/
/* decide if a manual page needs a RETURNS section. If   */
/* this is true, then output_identifier_description must */
/* be able to generate * sensible output for it          */
/*-------------------------------------------------------*/
...
void output_parameter_descriptions (ParameterList *params, char *function)
{
    ...
    {
	if (p->suppress || (!always_document_params && p->declarator->comment == NULL))
           continue;
    ...
    *description = *returns = NULL;
	...
	else
	{
		...
		if (terse)
		{
		    ...
		    endterse = c;
		    *terse = alloc_string(afterdash < endterse ? afterdash : start_line,
			                  endterse);
...
boolean params_identical(Parameter *first, Parameter *second)
{
    return first->decl_spec.flags == second->decl_spec.flags &&
	   ...
	   ((!first->decl_spec.text && !second->decl_spec.text)                                                          ||
	    (first->decl_spec.text && second->decl_spec.text && !strcmp(first->decl_spec.text, second->decl_spec.text))) &&

	   ((!first->declarator->text && !second->declarator->text) || (first->declarator->text && second->declarator->text &&
                    ...
                    const char *section)
{
	...
	output->code_start();
        ...
        /*-----------------------------------------------------*/ 
	/* list the include files the user asked us to include */
        /*-----------------------------------------------------*/ 
	...
	for (page = basepage; page; page = page->next)
	{
	    ...
	    output_declarator(page->declarator, 
		              page->declarator->head->params.first != page->declarator->head->params.last);
...
/* string to base the name on:                */
/*                                            */
/* this will be the name of an identifier or  */
/* the base of the input file name            */
/*--------------------------------------------*/

                                                       /*-----------------------------*/
char *page_file_name(const char         *based_on,     /* string filename is based on */
...
enum Output_Object page_output_type(DeclSpec *decl_spec, Declarator *declarator)
{
...
/*---------------------------------------*/
                                                      /*----------------------------------*/
void output_manual_pages(ManualPage    *first,        /* Head of manual page list         */
                                                      ...
                                                      /*----------------------------------*/
{
	...
	if (!tostdout)
	{
	    ...
	    filename = page_file_name( use_input_name && input_file_base ? input_file_base : page->declarator->name, output_type, section);
	...
	output_manpage(first, page, input_files, group_together && input_file_base ? input_file_base : page->declarator->name, group_together ? manual_section : section);
