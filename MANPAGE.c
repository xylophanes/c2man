/* $Id: manpage.c,v 2.0 2024-02-29 15:32:40 behdad Exp $
 ...
 *
 * Converted to ANSI-C by Mark O'Neill <man@tumblingdice.co.uk>
-------------------------------------------------------------------------*/ 
...
ManualPage *firstpage     = (ManualPage *)NULL;
ManualPage **lastpagenext = &firstpage;

void dummy(void)
{}

void new_manual_page(char       *comment,
                     DeclSpec   *decl_spec,
                     Declarator *declarator)
{
    ManualPage *newpage = (ManualPage *)NULL;
	...
	((decl_spec->flags & DS_EXTERN) && !header_file &&  declarator->type != DECL_FUNCDEF))
    {
...
{
    ManualPage *page  = (ManualPage *)NULL,
...
void remember_terse(char *comment)
{
    ...
    {   switch (state)
	{
	       ...
	       case LEADSPACE:	if (*c == '-')
               ...
               case DASH:       if (isspace(*c))
                                   state = TRAILSPACE;
                                else if (*c != '-')
                                   state = STUFF;
                                break;

               case TRAILSPACE: if (!isspace(*c))
                                {  c--; state = FOUND;
                                }
			        break;
...
/*-----------------------------------------------------------------------*/
                                                                                            /*-----------------------------*/
static void output_identifier_description( const char       *comment,                       /* comment for this identifier */
                                           ...
                                           const Declarator *declarator)
{
	...
	int        descriptions = 0;
	int        is_first     = 1;
	char       *longestag   = (char *)NULL;
	Enumerator *e           = (Enumerator *)NULL;
...
static boolean auto_documented(const ManualPage *page)
{
    ...
    /*-----------------------------------*/
                                                  /*----------------------------*/
    return page->decl_spec->enum_list != NULL;    /* enums are self-documenting */
                                                  /*----------------------------*/
}
...
{
    return (page->returns && page->returns[0])                              ||
...
boolean has_documented_parameters( const Declarator *d)
{
	...
	for (p = d->head->params.first; p != (Parameter *)NULL; p = p->next)
...
void output_parameter_descriptions ( ParameterList *params, char *function)
{
	...
	output_identifier_description(p->declarator->comment, output_comment, &p->decl_spec, p->declarator);
    }
...
{
    char *retstart = (char *)NULL;

    for (retstart = comment; retstart; retstart = strchr(retstart,'\n'))
	...
	{	    
	    ...
	    *description =  descend > comment ? alloc_string(comment,descend+1) : NULL;
	    retstart     += 7;
...
boolean split_function_comment(const char *comment,
                               ...
                               Section    **extra_sections)
{
    const char *c                = (char *)NULL,
               *start_text       = (char *)NULL,
               *end_text         = (char *)NULL;

    char **put_ptr               = (char **)NULL;

    Section *first_section       = (Section *)NULL,
            **lastnextsection    = &first_section;

    boolean explicit_description = FALSE;
    boolean lastblank            = TRUE;
    boolean skip_dash            = FALSE;
    
    *description = *returns = (char *)NULL;
    if (terse)
       *terse = (char *)NULL;
	...
	{   size_t section_len = c - start_line; /* length of section name */
	    ...
	    {   if (skip_dash)
                   start_text = skipdash(start_text);
	...
	else
	{
		...
		if (terse)
		{   const char *endterse = (char *)NULL,
                               *afterdash = skipdash(start_line);
		    ...
		    endterse = c;
		    *terse = alloc_string(afterdash < endterse ? afterdash : start_line, endterse);
	...
	{   while (*c && *c != '\n')
        ...
        /*-----------------------------------------------*/
        /* so far, the text ends at the end of this line */
        /*-----------------------------------------------*/
...
{
    return first->decl_spec.flags == second->decl_spec.flags &&
...
boolean mark_duplicate_parameters(ManualPage *firstpage)
{
    ...
    {   if (has_parameters(page->declarator))
	   for (param = page->declarator->head->params.first; param; param = param->next)
	   {   ManualPage *otherpage  = (ManualPage *)NULL;
	       Parameter  *otherparam = (Parameter *)NULL;
	    
	       if (always_document_params || param->declarator->comment)
	  	  any = TRUE;

	       for (otherpage = page->next; otherpage; otherpage = otherpage->next)
	       {
	  	    if (has_parameters(otherpage->declarator))
		    {  for (otherparam = otherpage->declarator->head->params.first; otherparam; otherparam = otherparam->next)
		       {
			     ...
			     if (param->declarator->comment)
                                has_comm |= US;

			     if (otherparam->declarator->comment)
                                has_comm |= THEM;
			     ...
			     switch(has_comm)
			     {
			           ...
			           case BOTH:
			                          if (!strcmp(param->declarator->comment, otherparam->declarator->comment))
				                     otherparam->suppress = TRUE;
			                          else
...
{
    ExcludeSection *exclude = (ExcludeSection *)NULL;

    for (exclude = first_excluded_section ; exclude ; exclude = exclude->next)
    {   if (!strcmp(section, exclude->name))
            return 1;
    }
                    ...
                    const char *section)
{
    ...
    {
	boolean explicit_description = split_function_comment(page->declarator->comment,
	...
	if (page->returns == NULL)
	{
		...
		{   char *newdesc = (char *)NULL;

		    if (split_returns_comment(page->description, &newdesc, &page->returns))
    ...
    output->header(basepage, input_files, grouped, title ? title : basepage->declarator->name, terseout, section);
	...
	{    output_format_string(decl_spec_prefix);
	    ...
	    output_declarator(page->declarator, page->declarator->head->params.first != page->declarator->head->params.last);
	    output->text(";\n");
	...
	for (page = basepage; page; page = page->next)
	{ if (has_parameters(page->declarator))
	     output_parameter_descriptions(&page->declarator->head->params, page->declarator->name);

	    if (!grouped)
                                     /*--------------------*/
   	       break;                /* only do first page */
                                     /*--------------------*/
	}
    }
	...
	{   if (needs_returns_section(page))
		...
		{   output->sub_section(page->declarator->name);
		    output_identifier_description(page->description, output_comment, page->decl_spec, page->declarator);
		}
	...
	if (!exclude_description)
	{   const char *descr = basepage->description ? basepage->description : terseout;

	    if (auto_documented(page) && !is_function_declarator(page->declarator))
		output_identifier_description(descr, output_comment, page->decl_spec, page->declarator);
	    else
	...
	{   if (needs_returns_section(page))
	    {  if (grouped) output->sub_section(page->declarator->name);
		  output_identifier_description(page->returns, output->returns, page->decl_spec, page->declarator);
	...
	{    next = section->next;
		...
		{   ManualPage *other_page = page->next;
			...
			for (;other_section; other_section = other_section->next)
			{    if (other_section->been_output || strcmp(other_section->name, section->name))
				continue;
	    ...
	    safe_free(section->name); // MAO was free()
	    safe_free(section->text);
	    safe_free(section);       // MAO was feee()
	}
    ...
    if (!group_together)
    {   ManualPage *also = (ManualPage *)NULL;
                     ...
                     const char *extension)
{
    ...
    if (subdir)
    {   if (filename)
           filename = strappend(filename, "/", NULLCP);
...
void output_manual_pages(ManualPage    *first,
                         ...
                         enum LinkType link_type)       // how grouped pages will be linked
{
    ...
    {   char *input_file_base = (char *)NULL;

	enum Output_Object output_type = page_output_type(page->decl_spec, page->declarator);
...
char tmpstr[64]; 
            ...
            (void)strcpy(tmpstr,page->declarator->name);
            (void)strcpy(page->declarator->name,page->declarator->text);
            (void)strcpy(page->declarator->text,tmpstr);
	...
	{   safe_free(filename);   // free previous, if any 
	    ...
	    filename = page_file_name( use_input_name && input_file_base ? input_file_base : page->declarator->text, output_type, section); // MAO was name
	    (void)fprintf(stderr,"generating: %s\n",filename);
	    ...
	    if (!remove_old_file(filename))
               exit(1);

	    if (freopen(filename, "w", stdout) == NULL)
	    {   my_perror("error opening output file", filename);
		safe_free(filename); // MAO was free()
		exit(1);
	    }
	}
	...
	output_manpage(first, page, input_files, group_together && input_file_base ? input_file_base : page->declarator->name,
    ...
    {  for (page=use_input_name && first->sourcefile ? first : first->next; page; page = page->next)
	{   enum Output_Object output_type = page_output_type(page->decl_spec, page->declarator);
	    const char *extension          = page_manual_section(output_type);
	    char *linkname                 = page_file_name(page->declarator->name, output_type, extension);
	    int result = 0;
	    ...
	    {   safe_free(linkname); // MAO was free()
		continue;
	    }
