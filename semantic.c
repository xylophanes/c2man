/* $Id: semantic.c,v 1.1 2024-06-03 09:57:40 behdad Exp $
 *
...
{
    #ifdef DEBUG
    (void)fprintf(stderr,"free_declarator: decl = %lx, name = %s, text = %s\n", (long int)d, d->name?d->name:"NULL", d->text?d->text:"NULL");
    #endif /* DEBUG */
...
void free_decl_list (DeclaratorList *decl_list)
{
    ...
    #ifdef DEBUG
    (void)fprintf(stderr,"free_decl_list: decl_list = %lx, first = %lx\n", (long int)decl_list, (long)decl_list->first);
    #endif /* DEBUG */
...
void new_parameter (Parameter *param,
                    DeclSpec *decl_spec,
...
void new_param_list (param_list, param)
...
void add_ident_list (ParameterList *to, ParameterList *from, Identifier *ident)
{
    ...
    new_parameter(p, (DeclSpec *)NULL, declarator, ident->comment_before, ident->comment_after);
...
void set_param_types (ParameterList *params, DeclSpec *decl_spec, DeclaratorList *declarators, char *comment, char *eolcomment)
{
	...
	{   output_error();
	    (void)fprintf(stderr, "declared argument \"%s\" is missing\n", d->name);
	} 
        else 
...
static void output_parameters (Declarator *d, boolean format)
{
	...
	if (format)
           output_format_string(first_param_prefix);
	output_parameter(p);
	...
	{   output->character(',');
	    if (format)
               output_format_string(middle_param_prefix);
	...
	if (format)
            output_format_string(last_param_suffix);
    }
    else
	output->text("void");
}
...
/*----------------------------------------------*/
                                                          ...
                                                          /*----------------------------*/
int remember_declarations (char           *comment,       /* comment before             */
                                                          ...
                                                          /*----------------------------*/
{
	...
	for (d = decl_list->first; d != NULL; d = next)
	{
	    ...
	    next = d->next;
            ...
            #ifdef DEBUG
	    (void)fprintf(stderr, "remember_declarations: text=%s name=%s\ncomment: %s\n", d->text,d->name, comment ? comment : "NULL");
            #endif /* DEBUG */

	    spec_copy      = *decl_spec;
	    spec_copy.text = strduplicate(decl_spec->text);
	    comment_copy   = d->comment ? d->comment : (comment ? strduplicate(comment) : NULL);
	    d->comment     = NULL;
